...
impl GameState {
    ...
    pub(crate) fn random_u8_inclusive(&mut self, start: u8, end: u8) -> u8 {
        self.rng.random_range(start..=end)
    }

    pub(crate) fn remember_recent_position(&mut self, index: usize, pos: Position) {
        remember_recent_position(&mut self.npcs[index].recent_positions, pos);
    }
    ...
    pub(crate) fn try_deliver_food_to_queen(&mut self, worker_index: usize) -> bool {
...
impl GameState {
    pub(crate) fn worker_lifespan_ticks_for(&self, index: usize) -> u16 {
    ...
    pub(crate) fn food_carry_max(&self) -> u16 {
        config_u16(&self.config, "colony.food_carry_max", 1).max(1)
    }

    pub(crate) fn adjacent_food_visible(&self, pos: Position) -> bool {
        !self.adjacent_food_positions(pos).is_empty()
    }

    pub(crate) fn adjacent_food_positions(&self, pos: Position) -> Vec<Position> {
        (-1..=1)
            .flat_map(|dy| (-1..=1).map(move |dx| (dx, dy)))
            .filter(|(dx, dy)| !(*dx == 0 && *dy == 0))
    ...
    pub(crate) fn search_behavior_profile(&self) -> SearchBehaviorProfile {
        match self
            .config
    ...
    pub(crate) fn effective_search_behavior_profile(
        &self,
        index: usize,
